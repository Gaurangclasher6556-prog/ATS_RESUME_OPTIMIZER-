...
def render_mock_interview_tab(company_name, target_role, job_description, need_resume_fn, need_jd_fn, safe_read_bytes_fn, extract_text_fn):
    ...
    if st.session_state["cf_stage"] == "Setup":
        ...
        st.markdown("**Write your code below — real sandboxed execution (Piston), "
                    "with AI fallback if offline:**")
        ...
        custom_stdin = st.text_input("Optional stdin for your run", key="cf_stdin")

        c_run, c_sub = st.columns(2)
        with c_run:
            if st.button("▶️ Run Code"):
                with st.spinner(f"Executing {cf_language} code..."):
                    res = ai_handler.run_code_real_or_simulated(
                        prob.get('description', ''), code,
                        language=cf_language, stdin=custom_stdin,
                    )
                    st.session_state["cf_term_out"] = res.get("terminal_output", "")
                    st.session_state["cf_term_real"] = res.get("real", False)

        if "cf_term_out" in st.session_state:
            if st.session_state.get("cf_term_real"):
                st.caption("✅ Real execution (Piston sandbox)")
            else:
                st.caption("⚠️ AI-simulated output (sandbox unavailable)")
