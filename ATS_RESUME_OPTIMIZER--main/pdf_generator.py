...
def generate_cover_letter_docx(text: str, name: str = "") -> bytes:
    """Render a cover letter (plain/markdown text) into a clean DOCX."""
    doc = Document()
    for sec in doc.sections:
    ...
    if name:
        h = doc.add_paragraph()
        _run(h, name, bold=True, size=14)

    for raw in (text or "").split("\n"):
        line = raw.strip()
        # Strip simple markdown emphasis markers for a clean letter.
