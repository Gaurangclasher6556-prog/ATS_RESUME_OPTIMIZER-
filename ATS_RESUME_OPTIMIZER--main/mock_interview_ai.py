...
def gather_realtime_intelligence(company, role):
    ...
    try:
        # The package was renamed from `duckduckgo_search` to `ddgs`. Import the
        # new name first, then fall back to the old one for compatibility.
        try:
            ...
            try:
                ...
                return (f"Real-time search unavailable (package not installed). "
                        f"Using AI knowledge for {company} {role} interview patterns.")
