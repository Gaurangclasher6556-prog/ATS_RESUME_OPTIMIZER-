...
def get_best_model() -> str:
    ...
    Cached with lru_cache so the (network) call to genai.list_models() runs only
    ONCE per process instead of on every single LLM request — previously this
    fired an extra round-trip before each generate_content call, adding 4+
    wasted network calls in the 4-pass optimizer alone.
    """
    try:
        available = {
            m.name.split("/")[-1]
...
def _parse_json(raw: str):
    """
    Strip markdown fences / conversational text and parse JSON safely.

    Returns the parsed object on success, or {} on failure. Hardened to tolerate
    common LLM JSON quirks: code fences, leading prose, and trailing commas.
    """
    ...
    # Extract the outermost JSON substring (object or array).
    ...
    try:
        ...
        # Second chance: remove trailing commas before } or ] and retry.
        repaired = re.sub(r",(\s*[}\]])", r"\1", raw)
        try:
            return json.loads(repaired)
        except json.JSONDecodeError as e:
            print(f"JSON Parse Error ({e}). Cleaned string was: {raw[:200]}...")
            return {}
...
def deep_pass2_bullets(resume_data: dict, job_description: str, keyword_analysis: dict) -> dict:
    ...
    kb_context = f"""
REFERENCE — GOLD STANDARD BULLET EXAMPLES (domain: {domain.replace('_', ' ')}):
{get_star_examples_prompt(domain)}
...
# ═══════════════════════════════════════════════════════════════════════════
#  HALLUCINATION / FABRICATION GUARDRAIL
# ═══════════════════════════════════════════════════════════════════════════

_NUM_RE = re.compile(r"\$\s?\d[\d,]*(?:\.\d+)?|\d+(?:\.\d+)?%?")


def _collect_numbers(resume_data: dict) -> set:
    """Collect every numeric token appearing in a resume's bullets & summary."""
    nums = set()
    text_parts = [resume_data.get("summary", "")]
    for exp in resume_data.get("experience", []) or []:
        text_parts += list(exp.get("bullets", []) or [])
    for proj in resume_data.get("projects", []) or []:
        text_parts += list(proj.get("bullets", []) or [])
    for part in text_parts:
        for m in _NUM_RE.findall(str(part)):
            nums.add(m.replace(" ", "").replace(",", ""))
    return nums


def detect_fabricated_metrics(original: dict, optimized: dict) -> list:
    """
    Return numeric claims present in the OPTIMIZED resume but absent from the
    ORIGINAL — likely metrics the AI invented. Enforces the "no fabrication"
    rule in code rather than trusting the prompt alone. Years and trivially
    small integers are ignored to reduce false positives.
    """
    orig_nums = _collect_numbers(original)
    new_nums = _collect_numbers(optimized)
    suspicious = []
    for n in sorted(new_nums - orig_nums):
        core = n.lstrip("$").rstrip("%")
        if core.isdigit():
            val = int(core)
            if val < 10 or (1900 <= val <= 2100):
                continue
        suspicious.append(n)
    return suspicious
...
def generate_cover_letter(resume_data: dict, job_description: str,
    ...
    """Generate a tailored, ATS-friendly cover letter in markdown."""
    prompt = f"""You are an expert career writer. Write a compelling, concise cover
letter (3-4 short paragraphs, ~250-320 words) for the candidate below.

TONE: {tone}
TARGET COMPANY: {company or 'the company'}
TARGET ROLE: {role or 'the role'}

RULES:
1. Open with a specific hook tied to the company/role — no generic "I am writing to apply".
2. Pull 2-3 concrete, quantified achievements from the resume that map to the JD.
3. Mirror key terminology from the job description naturally (ATS-friendly).
4. NEVER fabricate achievements, employers, or credentials the candidate lacks.
5. Close with a confident, forward-looking call to action.
6. Use real paragraphs (no bullet lists). Return clean markdown only.
...
def rewrite_resume_content(resume_data: dict) -> dict:
    ...
    # Infer the domain from the resume's own content so the rebuilder pulls the
    # most relevant gold-standard examples (no JD is provided in this flow).
    domain = detect_domain(json.dumps(resume_data))
    kb_context = f"""
REFERENCE EXAMPLES — GOLD STANDARD BULLETS (domain: {domain.replace('_', ' ')}):
{get_star_examples_prompt(domain)}
...
def run_code_real_or_simulated(question: str, code: str, language: str = "Python",
                               stdin: str = "") -> dict:
    """
    Execute the user's code for REAL via the Piston sandbox when possible.
    Falls back to the LLM-simulated judge only if the sandbox is unreachable or
    the language is unsupported. Returns {"terminal_output": str, "real": bool}.
    """
    try:
        import code_executor
        if code and code.strip() and code_executor.is_supported(language):
            result = code_executor.run_code(code, language=language, stdin=stdin)
            return {"terminal_output": code_executor.format_terminal(result),
                    "real": True}
    except Exception as e:
        print(f"Real execution failed, falling back to simulation: {e}")
    sim = simulate_code_run(question, code, language)
    sim["real"] = False
    return sim
