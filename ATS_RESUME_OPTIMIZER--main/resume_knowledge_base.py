...
def detect_domain(job_description: str) -> str:
    """
    Detect the most likely role domain from a JD and map it to a STAR-example
    domain key ('software_engineering', 'data_science', 'product_management',
    or 'general'). Used to route the right gold-standard bullet examples into
    the optimizer prompts instead of always defaulting to 'general'.
    """
    jd_lower = (job_description or "").lower()
    keyword_map = {
        "software_engineering": ["software engineer", "backend", "frontend",
                                  "full stack", "developer", "sde", "systems",
                                  "infrastructure", "web developer", "react",
                                  "devops", "sre", "platform engineer"],
        "data_science": ["data scientist", "machine learning", "ml engineer",
                         "data analyst", "deep learning", "analytics",
                         "data engineer", "nlp", "computer vision"],
        "product_management": ["product manager", "product owner",
                               "program manager", "technical program"],
    }
    scores = {dom: sum(1 for t in trig if t in jd_lower)
              for dom, trig in keyword_map.items()}
